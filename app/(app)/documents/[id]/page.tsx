...
function ActionBtn({ onClick, icon, label, bg, color }: any) {
  return (
...
export default function QuoteDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleConvertToInvoice = async () => {
    ...
    if (invoice) {
      await Promise.all([
        supabase.from('quote_items').insert(items.map(item => ({
          quote_id: invoice.id, name: item.name, description: item.description || '',
          item_type: item.item_type, quantity: item.quantity, unit_price: item.unit_price, line_total: item.line_total,
        }))),
  ...
  const handleDuplicate = async () => {
    ...
    if (newDoc) {
      await supabase.from('quote_items').insert(
        items.map(item => ({
          quote_id: newDoc.id, name: item.name, description: item.description || '',
          item_type: item.item_type, quantity: item.quantity, unit_price: item.unit_price, line_total: item.line_total,
        }))
      )
      clearCache('dashboard')
  ...
  const getStatusStyle = (status: string) => {
    const map: Record<string, string> = {
      ...
      rejected: 'badge-rejected', draft: 'badge-draft', expired: 'badge-overdue', partial: 'badge-partial',
    }
  ...
  if (!quote) {
    return (
      <div className='q-page'>
        <div className='q-loading'>Document not found</div>
      </div>
    )
  }
        ...
        {showPaymentForm && (
          <div className='q-card' style={{ padding: 20, marginBottom: 12 }}>
            ...
            <div className='q-form-group'>
              ...
              <input className='q-input' type='number' value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} onFocus={e => e.target.select()} min='0'/>
            </div>
            <div className='q-form-group'>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Item', 'Qty', 'Price', 'Total'].map((h, idx) => (
                  <th key={h} style={{ textAlign: idx === 0 ? 'left' : 'right', padding: '6px 0', fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        {isInvoice && payments.length > 0 && (
          <div className='q-card' style={{ padding: 20, marginBottom: 12 }}>
            ...
            {payments.map((payment, idx) => (
              <div key={payment.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '10px 0', borderBottom: idx < payments.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <div>
      ...
      <div style={{ position: 'fixed', right: 10, top: '40%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', gap: 8, zIndex: 50 }}>
        ...
        {client?.phone && (
          <ActionBtn onClick={handleWhatsApp}
            ...
            label='WA' bg='var(--green-bg)' color='var(--green)'
          />
        )}
        <ActionBtn onClick={() => router.push('/documents/' + params.id + '/edit')}
          ...
          label='Edit' bg='var(--orange-bg)' color='var(--orange)'
        />
        {isInvoice && !isFullyPaid && (
          <ActionBtn onClick={() => setShowPaymentForm(!showPaymentForm)}
            ...
            label='Pay' bg={showPaymentForm ? 'var(--green-bg)' : 'var(--white)'} color='var(--green)'
          />
        )}
        {!isInvoice && quote.status !== 'converted' && (
          <ActionBtn onClick={handleConvertToInvoice}
            ...
            label='Invoice' bg='var(--orange-bg)' color='var(--orange)'
          />
        )}
        {!isInvoice && quote.status === 'sent' && (
          <ActionBtn onClick={() => handleUpdateStatus('accepted')}
            ...
            label='Accept' bg='var(--green-bg)' color='var(--green)'
          />
        )}
        {!isInvoice && quote.status === 'sent' && (
          <ActionBtn onClick={() => handleUpdateStatus('rejected')}
            ...
            label='Reject' bg='var(--red-bg)' color='var(--red)'
          />
        )}
        <ActionBtn onClick={handleDuplicate}
          ...
          label='Copy' bg='var(--purple-bg)' color='var(--purple)'
        />
      </div>
